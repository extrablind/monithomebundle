    ...
    for (i = 0, ilen = (chart.data.datasets || []).length; i < ilen; ++i) {
      if (chart.isDatasetVisible(i)) {
        ...
        for (j = 0, jlen = data.length; j < jlen; ++j) {
          timestamp0 = parse(data[j].start, me);
          timestamp1 = parse(data[j].end, me);
          ...
          datasets[i][j] = [timestamp0, timestamp1, data[j].title];
...
Chart.controllers.timeline = Chart.controllers.bar.extend({
  ...
  update: function(reset) {
    ...
    updateElement: function(rectangle, index, reset) {
      ...
      var x = xScale.getPixelForValue(data.start);
      var end = xScale.getPixelForValue(data.end);
  ...
  tooltips: {
    callbacks: {
      ...
      label: function(tooltipItem, data) {
        ...
        return [d.title, moment(d.start).format('L LTS'), moment(d.end).format('L LTS')];
      }
    }
  }
};
